...
def adb_connect_firetv():
    ...
    button_ftv.configure(bg = "#e8fbe8")
    button_atv.configure(bg = "#f0f0f0")
    

def adb_connect_androidtv():
    execute("adb disconnect")
    execute("adb connect 192.168.10.101:5555")
    button_ftv.configure(bg = "#f0f0f0")
    button_atv.configure(bg = "#e8fbe8")
...
# Not in use. Need to find a way to re scan the network through cmd.
def connect_to_network(name):
    execute(f"netsh wlan connect name={name}")
    sleep(3)

button_ftv = create_button("Connect to FireTV", 20, [10, 30], adb_connect_firetv)
button_atv = create_button("Connect to AndroidTV", 20, [10, 70], adb_connect_androidtv)
